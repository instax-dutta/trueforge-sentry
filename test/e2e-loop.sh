...
for attempt in 1 2; do
  curl -sf -N --max-time 180 -X POST "$TF_URL/api/v1/sessions/$SID/turns" \
    -H "Content-Type: application/json" \
    -d '{"input":[{"type":"user.message","content":"Investigate payment-failures alert. You MUST call these tools in order: 1) Grafana query_prometheus for sum(rate(http_5xx_total[30m])) with datasource PBFA97CFB590B2093, 2) GitHub list_commits for instax-dutta/trueforge-sentry, 3) Then you MUST call sentry-lab lab_restore to propose the fix - this will trigger human approval, do NOT execute without it."}],"stream":true}' \
    > "$SESSION_LOG" 2>&1
